...
class Wheel {
	...
	endDisplay(winner, winnings) {
		...
		Battlelords.nameColor(winner, true) + ' has won ' + winnings + '</div>';
	}
}
	...
	wheel: {
		create: function (target, room, user) {
			...
			Economy.readMoney(user.userid, money => {
				...
				room.add('|raw|<b><font color="orange">' + Battlelords.nameColor(user, true) + ' is hosting the wheel.</font></b>');
				room.update();
		...
		join: function (target, room, user) {
			...
			Economy.readMoney(user.userid, money => {
				...
				Economy.writeMoney(user.userid, (room.wheel.wheel[room.wheel.wheel.length - 1] * room.wheel.wheelMulti) * -1, function () {
					room.add('|raw|<b><font color="orange">' + Battlelords.nameColor(user, true) + ' has spun the wheel.</font></b>');
			...
			delete room.wheel;
			room.add('|raw|<font color="orange">The WOM was ended by <b>' + Battlelords.nameColor(user, true) + '</b>.</font>'); //html needed
		},
