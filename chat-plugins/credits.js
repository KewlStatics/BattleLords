...
let shopTitle = 'Battle Lords Credit Shop';
...
Battlelords.readCredits = function (userid, callback) {
...
Battlelords.writeCredits = function (userid, amount, callback) {
	userid = toId(userid);
	Battlelords.database.all("SELECT * FROM users WHERE userid=$userid", {$userid: userid}, function (err, rows) {
		if (rows.length < 1) {
			Battlelords.database.run("INSERT INTO users(userid, credits) VALUES ($userid, $amount)", {$userid: userid, $amount: amount}, function (err) {
				if (err) return console.log(err);
				if (callback) return callback();
			});
		} else {
			amount += rows[0].credits;
			Battlelords.database.run("UPDATE users SET credits=$amount WHERE userid=$userid", {$amount: amount, $userid: userid}, function (err) {
				if (err) return console.log(err);
				if (callback) return callback();
			});
		}
...
exports.commands = {
	...
	creditatm: function (target, room, user) {
		...
		Battlelords.readCredits(userid, cred => {
			this.sendReplyBox(Battlelords.nameColor(target, true) + " has " + cred + ((cred === 1) ? " credits." : " credits."));
	...
	givecredits: function (target, room, user) {
		...
		Battlelords.writeCredits(targetUser, amount);
		...
		if (Users.get(targetUser) && Users.get(targetUser).connected) {
			Users.get(targetUser).popup("|modal||html|" + Battlelords.nameColor(user.name, true) + " has given you " + amount + ((amount === 1) ? " credit " : " credits "));
		}
	},
	...
	takecredits: function (target, room, user) {
		...
		if (Users.get(targetUser) && Users.get(targetUser).connected) {
			Users.get(targetUser).popup("|modal||html|" + Battlelords.nameColor(user.name, true) + " has taken " + amount + ((amount === 1) ? " credit " : " credits from you."));
		}
	},
		...
		Battlelords.readCredits(user.userid, cred => {
			...
			Battlelords.writeCredits(user.userid, -amount, () => {
				Battlelords.writeCredits(targetUser, amount, () => {
					...
					if (Users.getExact(targetUser) && Users.getExact(targetUser)) Users.getExact(targetUser).popup("|modal||html|" + Battlelords.nameColor(user.name, true) + " has sent you " + amount + ((amount === 1) ? " credit." : " credits."));
				});
			});
		});
	},
	...
	claim: function (target, room, user) {
		...
		Battlelords.readCredits(user.userid, userCred => {
			...
			case 'redticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Red Ticket.");
			...
			case 'cyanticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Cyan Ticket.");
			...
			case 'blueticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Blue Ticket.");
			...
			case 'orangeticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased an Orange Ticket.");
			...
			case 'silverticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Silver Ticket.");
			...
			case 'violetticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Violet Ticket.");
			...
			case 'yellowticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Yellow Ticket.");
			...
			case 'whiteticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a White Ticket.");
			...
			case 'greenticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Green Ticket.");
			...
			case 'crystalticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Crystal Ticket.");
			...
			case 'goldticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Gold Ticket.");
			...
			case 'blackticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Black Ticket.");
			...
			case 'rubyticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Ruby Ticket.");
			...
			case 'sapphireticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Sapphire Ticket.");
			...
			case 'magentaticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Magenta Ticket.");
			...
			case 'rainbowticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased a Rainbow Ticket.");
			...
			case 'emeraldticket':
				...
				Battlelords.messageSeniorStaff(user.name + " has purchased an Emerald Ticket.");
				Rooms.get('marketplacestaff').add('|c|~Credit Shop Alert|**' + user.name + " has purchased a Rainbow Ticket.**");
	...
	credits: function (target, room, user) {
		...
		Battlelords.database.all("SELECT SUM(credits) FROM users;", (err, rows) => {
			if (err) return console.log("credits1: " + err);
			let totalCredits = rows[0]['SUM(credits)'];
			Battlelords.database.all("SELECT userid, SUM(credits) AS total FROM users GROUP BY credits HAVING TOTAL > 0;", (err, rows) => {
				if (err) return console.log("credits2: " + err);
				let userCount = rows.length;
				Battlelords.database.all("SELECT * FROM users ORDER BY credits DESC LIMIT 1;", (err, rows) => {
					...
					Battlelords.database.all("SELECT AVG(credits) FROM users WHERE credits > 0;", (err, rows) => {
	...
	luckiestusers: function (target, room, user) {
		...
		Battlelords.database.all("SELECT userid, credits, name FROM users ORDER BY credits DESC LIMIT $target;", {$target: target}, function (err, rows) {
