'use strict';
	...
	Battlelords.tourLadder = new sqlite3.Database('config/tourladder.db', () => {
		Battlelords.tourLadder.run('CREATE TABLE IF NOT EXISTS tourladder (userid VARCHAR(18), elo FLOAT, wins INTEGER, losses INTEGER, ties INTEGER, tourwins INTEGER);');
	});
	let deleteLadderConfirm = false;
	
	Battlelords.updateTourLadder = function (p1, p2, result, room) {
		Battlelords.tourLadder.all("SELECT * FROM tourladder WHERE userid = '" + p1.userid + "' OR userid = '" + p2.userid + "';", (err, users) => {
			...
			Battlelords.tourLadder.run(sql1, () => {
				Battlelords.tourLadder.run(sql2, () => {
					...
					room.addRaw('<br />' + Battlelords.nameColor(p1.name, true) + '\'s rating: ' + Math.round(p1elo) + ' &rarr; <strong>' + Math.round(newP1Elo) + '</strong><br />(' + reasons + ')');
	
					reasons = '' + (Math.round(newP2Elo) - Math.round(p2elo)) + ' for ' + (result === 'win' ? 'losing' : (result === 'loss' ? 'winning' : 'tying'));
					if (reasons.charAt(0) !== '-') reasons = '+' + reasons;
					room.addRaw(Battlelords.nameColor(p2.name, true) + '\'s rating: ' + Math.round(p2elo) + ' &rarr; <strong>' + Math.round(newP2Elo) + '</strong><br />(' + reasons + ')');
	...
	Battlelords.addTourWin = function (user) {
		Battlelords.tourLadder.run("UPDATE tourladder SET tourwins = tourwins + 1 WHERE userid = '" + toId(user) + "';");
	};
	...
	exports.commands = {
		...
		tourladder: function (target, room, user) {
			...
			if (!target || !target.trim()) {
				Battlelords.tourLadder.all("SELECT * FROM tourladder ORDER BY elo DESC;", (err, users) => {
					if (err) return console.log(err);
					if (!users.length) {
					...
					for (let i = 0; i < (this.broadcasting && users.length > 10 ? 10 : users.length); i++) {
						...
						table += '<tr><td><center>' + (i + 1) + '</center></td><td style = "text-align: center">' + Battlelords.nameColor(name, true) + '</td><td style = "text-align: center">' + Math.round(users[i].elo) + '</td>' +
			...
			Battlelords.tourLadder.all("SELECT elo FROM tourladder WHERE userid = '" + toId(target) + "';", (err, users) => {
				if (err) return console.log(err);
				if (!users.length) {
					this.sendReplyBox(Battlelords.nameColor(target, true) + ' has not played any rated tournaments yet.');
				} else {
					this.sendReplyBox(Battlelords.nameColor(target, true) + '\'s Tournament Elo is <b>' + Math.round(users[0].elo) + '</b>.');
				}
				room.update();
			});
		},
		...
		tourladderusers: function (target, room, user) {
			if (!this.runBroadcast()) return;
			Battlelords.tourLadder.all("SELECT COUNT(*) FROM tourladder;", (err, count) => {
				if (err) return console.log(err);
				count = count[0]['COUNT(*)'];
				if (!count) {
					this.sendReplyBox('No rated tournaments have been played yet.');
		...
		resettourladder: function (target, room, user) {
			...
			Battlelords.tourLadder.all("SELECT * FROM tourladder;", (err, users) => {
				if (err) return console.log(err);
				...
				Battlelords.tourLadder.run("DELETE FROM tourladder;", () => {
