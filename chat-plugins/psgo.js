...
Battlelords.tourCard = function (tourSize, userid) {
...
exports.commands = {
	psgo: {
		...
		manager: {
			...
			add: function (target, room, user) {
				...
				if (Users(target) && Users(target).connected) Users(target).send("|popup||html|" + Battlelords.nameColor(user.name) + " has set you as a card manager.");
			...
			delete: function (target, room, user) {
				...
				if (Users(target) && Users(target).connected) Users(target).send("|popup||html|" + Battlelords.nameColor(user.name) + " has removed your card manager status.");
			...
			list: function (target, room, user) {
				...
				let output = [];
				for (let u in managers) output.push(Battlelords.nameColor(managers[u], true));
	...
	showcase: function (target, room, user) {
		...
		this.sendReplyBox('<div style="max-height: 300px; overflow-y: scroll;">' + cardsMapping.join('') + '</div><br><center><b><font color="' + Battlelords.hashColor(userid) + '">' + userid + '</font> has ' + cards.length + ' cards and ' + getPointTotal(userid) + ' points.</b></center>');
	},
