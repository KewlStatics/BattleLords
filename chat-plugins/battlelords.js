...
let colorCache = {};
BattleLords.customColors = {};
let regdateCache = {};
Users.vips = [];
let Advertisements = {};

BattleLords.autoJoinRooms = {};
try {
	BattleLords.autoJoinRooms = JSON.parse(fs.readFileSync('config/autojoin.json', 'utf8'));
} catch (e) {}

BattleLords.tells = {};
try {
	BattleLords.tells = JSON.parse(fs.readFileSync('config/tells.json', 'utf*'));
} catch (e) {}
const mainColors = {
	'theimmortal': 'taco',
	'bmelts': 'testmelts',
...
exports.commands = {
	...
	seen: function (target, room, user) {
		...
		case '!name':
			...
			BattleLords.database.all("SELECT * FROM users WHERE lastSeen NOT NULL", (err, rows) => {
		...
		default:
			...
			let userName = '<strong class="username">' + BattleLords.nameColor(target, false) + '</strong>';
			...
			BattleLords.lastSeen(userid, seen => {
	...
	regdate: function (target, room, user, connection) {
		...
		if (!this.runBroadcast()) return;
		BattleLords.regdate(target, date => {
			this.sendReplyBox(BattleLords.nameColor(target, false) + (date ? " was registered on " + moment(date).format("dddd, MMMM DD, YYYY HH:mmA ZZ") : " is not registered."));
			room.update();
		});
	},
	...
	tell: function (target, room, user, connection, cmd) {
		...
		if (BattleLords.tells[id]) {
			if (!user.can('hotpatch')) {
				let names = Object.keys(user.prevNames).concat(user.userid);
				for (let i in names) {
					let name = names[i];
					if (BattleLords.tells[id][name] && BattleLords.tells[id][name].length >= MAX_TELLS) return this.sendReply('You may only leave ' + MAX_TELLS + ' messages for a user at a time. Please wait until ' + targetUser + ' comes online and views them before sending more.');
				}
			}
		} else {
			BattleLords.tells[id] = {};
		}

		let tell = BattleLords.tells[id][user.userid];
		let msg = '<span style = "color:gray;"><i>(Sent by ' + user.name + ' on ' + moment().format("ddd, MMMM DD, YYYY HH:mmA ZZ") + ')</i></span><br><b><span style = "color:' + BattleLords.hashColor(user.userid) + '">' + user.name + ':</span></b> ' + Tools.escapeHTML(target);
		if (tell) {
			BattleLords.tells[id][user.userid].push(msg);
		} else {
			BattleLords.tells[id][user.userid] = [msg];
		}

		fs.writeFileSync('config/tells.json', JSON.stringify(BattleLords.tells));
	...
	away: function (target, room, user, connection, cmd) {
		...
		if (user.can('mute', null, room)) this.add("|raw|-- <font color='" + BattleLords.hashColor(user.userid) + "'><strong>" + Tools.escapeHTML(user.name) + "</strong></font> is now " + target.toLowerCase() + ".");
	...
	back: function (target, room, user) {
		...
		if (statusIdx < 0) {
			user.isAway = false;
			if (user.can('mute', null, room)) this.add("|raw|-- <font color='" + BattleLords.hashColor(user.userid) + "'><strong>" + Tools.escapeHTML(user.name) + "</strong></font> is no longer away.");
			return false;
		}
		...
		user.updateIdentity();
		user.isAway = false;
		if (user.can('mute', null, room)) this.add("|raw|-- <font color='" + BattleLords.hashColor(user.userid) + "'><strong>" + Tools.escapeHTML(newName) + "</strong></font> is no longer " + status.toLowerCase() + ".");
	...
	hex: function (target, room, user) {
		...
		this.sendReplyBox('The hex code of ' + BattleLords.nameColor(targetUser, true) + ' is: <font color="' + BattleLords.hashColor(targetUser) + '"><b>' + BattleLords.hashColor(targetUser) + '</b></font>');
	},
	...
	title: function (target, room, user) {
		...
		case "set":
			...
			BattleLords.setTitle(targetUser, title);
			if (Users(targetUser).connected) Users(targetUser).popup("|html|" + BattleLords.nameColor(user.name) + " has set your user title to \"" + title + "\".");
			this.sendReply("|raw|You've set " + BattleLords.nameColor(targetUser) + "'s title to \"" + title + "\".");
			Rooms('upperstaff').add("|raw|" + BattleLords.nameColor(user.name, true) + " has set " + BattleLords.nameColor(targetUser, true) + "'s user title to " + title + ".").update();
			BattleLords.messageSeniorStaff("/html " + BattleLords.nameColor(user.name, true) + " has set " + BattleLords.nameColor(targetUser, true) + "'s user title to " + title + ".");
			break;
		case "delete":
			if (!this.can('title')) return false;
			if (!targets[1]) return this.parse("/help title");
			BattleLords.getTitle(targetUser, title => {
				if (title === "") return this.sendReply(targetUser + " does not have a title.");
				BattleLords.setTitle(targetUser, "", () => {
					if (Users(targetUser) && Users(targetUser).connected) Users(targetUser).popup("|html|" + BattleLords.nameColor(user.name) + " has removed your user title.");
					this.sendReply("You have removed " + targetUser + "'s user title.");
					Rooms('upperstaff').add("|raw|" + BattleLords.nameColor(user.name, true) + " has removed " + BattleLords.nameColor(targetUser, true) + "'s user title.").update();
					BattleLords.messageSeniorStaff("/html " + BattleLords.nameColor(user.name, true) + " has removed " + BattleLords.nameColor(targetUser, true) + "'s user title.");
				});
			});
			break;
		case "view":
			...
			BattleLords.getTitle(targetUser, title => {
				if (title === "") {
					this.sendReplyBox(BattleLords.nameColor(targetUser, true) + " does not have a title.");
				} else {
					this.sendReplyBox(BattleLords.nameColor(targetUser, true) + "'s user title is \"" + title + "\".");
				}
				room.update();
			});
			break;
	...
	advertise: function (target, room, user, connection) {
		...
		if (!room.lastAdvertisement) {
			room.add('|raw|<div class="infobox"><strong><font color=#2DA900>Advertisement: </font></strong> ' + BattleLords.parseMessage(message) + ' - ' + BattleLords.nameColor(user.name) + '</div>');
			room.update();
...
Object.assign(BattleLords, {
	hashColor: function (name) {
		name = toId(name);
		if (mainColors[name]) name = mainColors[name];
		if (BattleLords.customColors[name]) return BattleLords.customColors[name];
	...
	updateSeen: function (userid) {
		...
		BattleLords.database.all("SELECT * FROM users WHERE userid=$userid", {$userid: userid}, function (err, rows) {
			if (rows.length < 1) {
				BattleLords.database.run("INSERT INTO users(userid, lastSeen) VALUES ($userid, $date)", {$userid: userid, $date: date}, function (err) {
					if (err) return console.log(err);
				});
			} else {
				BattleLords.database.run("UPDATE users SET lastSeen=$date WHERE userid=$userid", {$date: date, $userid: userid}, function (err) {
					if (err) return console.log(err);
				});
			}
		});
	...
	reloadCSS: function () {
		let options = {
			...
			port: 80,
			path: '/customcss.php?server=BattleLords',
			method: 'GET',
		};
		http.get(options);
	},
	...
	saveAutoJoins: function () {
		fs.writeFileSync('config/autojoin.json', JSON.stringify(BattleLords.autoJoinRooms));
	},

	getTells: function (user) {
		let tell = BattleLords.tells[user.userid];
		if (!tell) return;
		for (let i in tell) {
			tell[i].forEach(msg => user.send('|pm| Tells|' + user.getIdentity() + '|/raw ' + msg));
		}
		delete BattleLords.tells[user.userid];
		fs.writeFileSync('config/tells.json', JSON.stringify(BattleLords.tells));
	},
	...
	getTitle: function (userid, callback) {
		...
		userid = toId(userid);
		BattleLords.database.all("SELECT title FROM users WHERE userid=$userid", {$userid: userid}, function (err, rows) {
			if (err) return console.log(err);
	...
	setTitle: function (userid, title, callback) {
		...
		BattleLords.database.all("SELECT * FROM users WHERE userid=$userid", {$userid: userid}, function (err, rows) {
			if (rows.length < 1) {
				BattleLords.database.run("INSERT INTO users(userid, title) VALUES ($userid, $title)", {$userid: userid, $title: title}, function (err) {
					if (err) return console.log(err);
					if (callback) return callback();
				});
			} else {
				BattleLords.database.run("UPDATE users SET title=$title WHERE userid=$userid", {$title: title, $userid: userid}, function (err) {
					if (err) return console.log(err);
					if (callback) return callback();
				});
			}
...
if (!Config.advertisementTimer) {
	Config.advertisementTimer = setInterval(function () {
		...
		Rooms('lobby').add('|raw|<div class="infobox"><strong><font color="#2DA900">Advertisement: </font></strong> ' + BattleLords.parseMessage(message) + ' - ' + BattleLords.nameColor(user) + '</div>');
		Rooms('lobby').update();
