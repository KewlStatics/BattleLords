...
Battlelords.ignoreEmotes = {};
try {
	Battlelords.ignoreEmotes = JSON.parse(fs.readFileSync('config/ignoreemotes.json', 'utf8'));
} catch (e) {}
...
function parseEmoticons(user, message, room) {
	if (emoteRegex.test(message)) {
		message = Battlelords.parseMessage(message).replace(emoteRegex, function (match) {
...
Battlelords.parseEmoticons = parseEmoticons;
...
exports.commands = {
	...
	emoticon: function (target, room, user) {
		...
		case 'add':
			...
			Rooms('upperstaff').add('|raw|' + Battlelords.nameColor(user.name, true) + ' has added the emote "' + Tools.escapeHTML(parts[1]) +
				'": <img width="40" height="40" src="' + parts[2] + '">').update();
			Battlelords.messageSeniorStaff('/html ' + Battlelords.nameColor(user.name, true) + ' has added the emote "' + Tools.escapeHTML(parts[1]) +
		...
		case 'ignore':
			if (Battlelords.ignoreEmotes[user.userid]) return this.errorReply("You are already ignoring emoticons.");
			Battlelords.ignoreEmotes[user.userid] = true;
			fs.writeFileSync('config/ignoreemotes.json', JSON.stringify(Battlelords.ignoreEmotes));
			this.sendReply("You are now ignoring emoticons.");
			break;

		case 'unignore':
			if (!Battlelords.ignoreEmotes[user.userid]) return this.errorReply("You aren't ignoring emoticons.");
			delete Battlelords.ignoreEmotes[user.userid];
			fs.writeFileSync('config/ignoreemotes.json', JSON.stringify(Battlelords.ignoreEmotes));
