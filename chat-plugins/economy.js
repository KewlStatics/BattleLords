...
Battlelords.database = new sqlite3.Database('config/users.db', function () {
	Battlelords.database.run("CREATE TABLE if not exists users (userid TEXT, name TEXT, bucks INTEGER, lastSeen INTEGER, onlineTime INTEGER, credits INTEGER, title TEXT)");
});
...
let shopTitle = 'Battle Lords Shop';
let serverIp = 'battlekek-kewlstatics.c9users.io';
...
let Economy = global.Economy = {
	readMoney: function (userid, callback) {
		...
		userid = toId(userid);
		Battlelords.database.all("SELECT * FROM users WHERE userid=$userid", {$userid: userid}, function (err, rows) {
	...
	writeMoney: function (userid, amount, callback) {
		...
		Battlelords.database.all("SELECT * FROM users WHERE userid=$userid", {$userid: userid}, function (err, rows) {
			if (rows.length < 1) {
				Battlelords.database.run("INSERT INTO users(userid, bucks) VALUES ($userid, $amount)", {$userid: userid, $amount: amount}, function (err) {
					if (err) return console.log(err);
					if (callback) return callback();
				});
			} else {
				amount += rows[0].bucks;
				Battlelords.database.run("UPDATE users SET bucks=$amount WHERE userid=$userid", {$amount: amount, $userid: userid}, function (err) {
					if (err) return console.log(err);
					if (callback) return callback();
				});
			}
...
exports.commands = {
	...
	wallet: function (target, room, user) {
		...
		Economy.readMoney(userid, money => {
			this.sendReplyBox(Battlelords.nameColor(target, true) + " has " + money + ((money === 1) ? " buck." : " bucks."));
	...
	givebucks: function (target, room, user) {
		...
		if (Users(targetUser) && Users(targetUser).connected) {
			...
			' from ' + Battlelords.nameColor(user.userid, true) + '.');
		}
	...
	takebucks: function (target, room, user) {
		...
		Economy.writeMoney(targetUser, -amount);
		if (Users(targetUser) && Users(targetUser).connected) {
			Users.get(targetUser).popup('|html|' + Battlelords.nameColor(user.userid, true) + ' has removed ' + amount + ' ' + (amount === 1 ? 'buck' : 'bucks') +
			' from you.<br />');
		}
		...
		Economy.readMoney(user.userid, money => {
			...
			Economy.writeMoney(user.userid, -amount, () => {
				Economy.writeMoney(targetUser, amount, () => {
					...
					if (Users.getExact(targetUser) && Users.getExact(targetUser)) Users.getExact(targetUser).send('|popup||html|' + Battlelords.nameColor(user.name, true) + " has sent you " + amount + ((amount === 1) ? " buck." : " bucks."));
				});
			});
		});
	},
	...
	buy: function (target, room, user) {
		...
		Economy.readMoney(user.userid, userMoney => {
			...
			case 'avatar':
				...
				Battlelords.messageSeniorStaff("/html " + Battlelords.nameColor(user.name, true) + " has purchased an avatar.<center><img src=\"" + targetSplit[1] +
				...
				Rooms('upperstaff').add("|raw|" + Battlelords.nameColor(user.name, true) + " has purchased an avatar.").update();
			...
			case 'room':
				...
				Battlelords.messageSeniorStaff("/html " + Battlelords.nameColor(user.name, true) + " has purchased a chat room. Name: " + Tools.escapeHTML(targetSplit[1]));
				Rooms('upperstaff').add("|raw|" + Battlelords.nameColor(user.name, true) + " has purchased a chat room named \"" + Tools.escapeHTML(targetSplit[1]) + "\".").update();
			...
			case 'poof':
				...
				Battlelords.messageSeniorStaff("/html " + Battlelords.nameColor(user.name, true) + " has purchased a poof message. Message: " + Tools.escapeHTML(targetSplit[1]));
				Rooms('upperstaff').add("|raw|" + Battlelords.nameColor(user.name, true) + " has purchased a poof message. Message: " + Tools.escapeHTML(targetSplit[1])).update();
			...
			case 'title':
				...
				Battlelords.messageSeniorStaff("/html " + Battlelords.nameColor(user.name, true) + " has purchased a title: <b><font color=\"" + (hex ? hex : "#b30000") + "\">" +
				...
				Rooms('upperstaff').add("|raw|" + Battlelords.nameColor(user.name, true) + " has purchased a title: <b><font color=\"" + (hex ? hex : "#b30000") + "\">" +
			...
			case 'infobox':
				...
				Battlelords.messageSeniorStaff("/html " + Battlelords.nameColor(user.name, true) + " has purchased an infobox.");
				Rooms('upperstaff').add("|raw|" + Battlelords.nameColor(user.name, true) + " has purchased an infobox.").update();
			...
			case 'declare':
				...
				Battlelords.messageSeniorStaff("/html " + Battlelords.nameColor(user.name, true) + " has purchased a declare. Message: " + Tools.escapeHTML(targetSplit[1]));
				Rooms('upperstaff').add("|raw|" + Battlelords.nameColor(user.name, true) + " has purchased a declare. Message: " + Tools.escapeHTML(targetSplit[1])).update();
			...
			case 'fix':
				...
				Battlelords.messageSeniorStaff("/html " + Battlelords.nameColor(user.name, true) + " has purchased a fix.");
				Rooms('upperstaff').add("|raw|" + Battlelords.nameColor(user.name, true) + " has purchased a fix.").update();
			...
			case 'emote':
				...
				Economy.logTransaction(user.name + " has purchased an emote for " + prices[itemid] + " bucks. Name: " + Tools.escapeHTML(targetSplit[1]) +
					" Image: " + targetSplit[2]);
				Battlelords.messageSeniorStaff("/html " + Battlelords.nameColor(user.name, true) + " has purchased an emote. Name: " + Tools.escapeHTML(targetSplit[1]) +
				...
				Rooms('upperstaff').add("|raw|" + Battlelords.nameColor(user.name, true) + " has purchased an emote. Name: " + Tools.escapeHTML(targetSplit[1]) +
			...
			case 'icon':
				...
				Rooms('upperstaff').add("|raw|" + Battlelords.nameColor(user.name, true) + " has purchased an icon. <img src=\"" + targetSplit[1] + "\" width=\"32\" height=\"32\">").update();
				Battlelords.messageSeniorStaff("/html " + Battlelords.nameColor(user.name, true) + " has purchased an icon. <img src=\"" + targetSplit[1] + "\" width=\"32\" height=\"32\">" +
			...
			case 'color':
				...
				Battlelords.messageSeniorStaff("/html " + Battlelords.nameColor(user.name, true) + " has purchased a custom color. Color: <font color=\"" + targetSplit[1] +
					"\">" + Tools.escapeHTML(user.name) + "</font>" +
					"<br /><button name=\"send\" value=\"/customcolor " + user.name + ", " + targetSplit[1] + "\">Click to add</button>");
				Rooms('upperstaff').add("|raw|" + Battlelords.nameColor(user.name, true) + " has purchased a custom color. Color: <font color=\"" + targetSplit[1] +
	...
	bucks: function (target, room, user) {
		...
		Battlelords.database.all("SELECT SUM(bucks) FROM users;", (err, rows) => {
			if (err) return console.log("bucks1: " + err);
			let totalBucks = rows[0]['SUM(bucks)'];
			Battlelords.database.all("SELECT userid, SUM(bucks) AS total FROM users GROUP BY bucks HAVING TOTAL > 0;", (err, rows) => {
				if (err) return console.log("bucks2: " + err);
				let userCount = rows.length;
				Battlelords.database.all("SELECT * FROM users ORDER BY bucks DESC LIMIT 1;", (err, rows) => {
					...
					Battlelords.database.all("SELECT AVG(bucks) FROM users WHERE bucks > 0;", (err, rows) => {
	...
	richestuser: function (target, room, user) {
		...
		function showResults(rows) {
			...
			for (let u in rows) {
				...
				output += '<tr><td>' + count + '</td><td>' + Battlelords.nameColor(username, true) + '</td><td>' + rows[u].bucks + '</td></tr>';
				count++;
			}
		...
		Battlelords.database.all("SELECT userid, bucks, name FROM users ORDER BY bucks DESC LIMIT $target;", {$target: target}, function (err, rows) {
	...
	profile: function (target, room, user) {
		...
		Economy.readMoney(userid, bucks => {
			Battlelords.regdate(userid, date => {
				if (date) regdate = regdate = moment(date).format("MMMM DD, YYYY");
				Battlelords.lastSeen(userid, online => {
					Battlelords.getTitle(userid, title => {
			...
			function showProfile(bucks, regdate, lastOnline, title) {
				...
				profile += '&nbsp;<font color=#b30000><b>Name: </font>' + Battlelords.nameColor(userid, true) + (title === "" ? "" : " (" + title + ")") + flag + '<br />';
				...
				if (Battlelords.getLeague(userid)) profile += '&nbsp;<font color=#b30000><b>League: </font></b>' + Battlelords.getLeague(userid) + '<br />';
