...
function load() {
	fs.readFile('config/customcolors.json', 'utf8', function (err, file) {
		if (err) return;
		Battlelords.customColors = JSON.parse(file);
	});
}
...
function updateColor() {
	fs.writeFileSync('config/customcolors.json', JSON.stringify(Battlelords.customColors));

	let newCss = '/* COLORS START */\n';

	for (let name in Battlelords.customColors) {
		newCss += generateCSS(name, Battlelords.customColors[name]);
	}
	newCss += '/* COLORS END */\n';
...
exports.commands = {
	...
	customcolor: function (target, room, user) {
		...
		if (target[1] === 'delete') {
			if (!Battlelords.customColors[toId(target[0])]) return this.errorReply('/customcolor - ' + target[0] + ' does not have a custom color.');
			delete Battlelords.customColors[toId(target[0])];
			updateColor();
		...
		Battlelords.customColors[toId(target[0])] = target[1];
		updateColor();
	},
