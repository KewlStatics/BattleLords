...
function updateIcons() {
	...
	Battlelords.reloadCSS();
}
Battlelords.updateIcons = updateIcons;
...
exports.commands = {
	...
	icon: function (target, room, user) {
		...
		if (target[1] === 'delete') {
			...
			if (Users(target[0]) && Users(target[0]).connected) Users(target[0]).popup("|modal||html|" + Battlelords.nameColor(user.name, true) + " removed your icon.");
			return;
		}

		this.sendReply("|raw|You have given <b><font color=" + Battlelords.hashColor(Tools.escapeHTML(target[0])) + ">" + Tools.escapeHTML(target[0]) + "</font></b> an icon.");
		if (Users(target[0]) && Users(target[0]).connected) Users(target[0]).popup("|modal||html|" + Battlelords.nameColor(user.name, true) + " has set your icon as : <img src='" + target[1] + "' width='32' height='32'");
		Rooms('upperstaff').add('|raw|<b><font color="' + Battlelords.hashColor(Tools.escapeHTML(target[0])) + '">' + Tools.escapeHTML(target[0]) + '</font> has received an icon from ' + Tools.escapeHTML(user.name) + '.</b>').update();
