...
exports.commands = {
	...
	viewlogs2: function (target, room, user, connection, cmd) {
		...
		fs.readFile('logs/chat/' + targetRoom.toLowerCase() + '/' + splitDate[0] + '-' + splitDate[1] + '/' + date + '.txt', 'utf8', (err, data) => {
			...
			data = targetRoom + "|" + date + "|" + JSON.stringify(Battlelords.customColors) + "\n" + data;
...
function parseMessage(message, user) {
	...
	case 'c':
		...
		'</small><b><font color="' + Battlelords.hashColor(name.substr(1)) + '">' + name.substr(1, name.length) + ':</font></b><em>' +
		Battlelords.parseMessage(lineSplit.slice(3).join('|')) + '</em></span>';
		break;
	case 'uhtml':
